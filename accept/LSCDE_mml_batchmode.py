...
from LSCDE import LSCDE_CV  # , LSCDE_CV_new
from fastkde_mml import set_mask, cut_alt
...
sigma_grid_var2 = np.array([0.02, 0.2, 2])
sigma_grid_var3 = np.array([0.0003, 0.003, 0.03])
sigma_grid_var4 = np.array([0.0005, 0.005, 0.05])
sigma_grid_var5 = np.array([0.0005, 0.005, 0.05])
sigma_grids_list = [sigma_grid_var2, sigma_grid_var3,
                    sigma_grid_var4, sigma_grid_var5]


def compute_pdf_scoring(data, h, FNUM, n_batch, test_datas, test_h_list=None,
    ...
    data : pandas DataFrame, shape = (n_samples, 4)
        Dataframe containing observations of variable 2 to 5 for all
    ...
    h_min = h_train.min()
    h_max = h_train.max()
    h_arr = np.linspace(h_min, h_max, n_batch)
    h1_arr = h_arr[:-1]
    h2_arr = h_arr[1:]
    ...
    for window in h_windows:
        ...
        cut_h_train = cut_alt(h_train, h_train, h1, h2)
        ...
        vprint("Training...\n", verbose)
        start = time()
        for sigma_grid, train_i in zip(sigma_grids_list, train_iterator):
            ...
            #model = LSCDE_CV_new()
            model = LSCDE_CV()
            model.fit(cut_h_train, train_i, sigma_range=sigma_grid,
                      lambda_range=10 ** np.linspace(-3, 1, 5),
                      kernel_num=min(cut_h_train.shape[0], 100))

            pdfs_per_dim.append(model)
        training_time_per_h.append(time() - start)
        ...
        for h_test, test in zip(h_test_list, test_list):
            ...
            cut_h_test = cut_alt(h_test, h_test, h1, h2)
            if isinstance(cut_test, pd.DataFrame):
                cut_test = cut_test.values
            if isinstance(cut_h_test, pd.DataFrame):
                cut_h_test = cut_h_test.values
...
#            print(len(pdfs_per_dim))
            for i, (train_i, test_i, model) in enumerate(zip(train_iterator, test_iterator, pdfs_per_dim)):
                ...
                if test_i.shape[0] > 0:
                    pdf_at_points = model.predict(cut_h_test, test_i)
                    if normalize_pdfs:
                        pdf_at_points /= model.max(cut_h_test)
                else:
        ...
        pdf_per_test_data_per_h.append(pdf_per_test_data)
    ...
    training_time = np.sum(training_time_per_h)
